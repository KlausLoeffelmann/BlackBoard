//---------------------------------------------------------------------
// <copyright file="SortableBindingList`.cs" company="Microsoft">
    ...
    internal class SortableBindingList<T> : BindingList<T>
    {
        ...
        internal class PropertyComparer : Comparer<T>
        {
            ...
            public PropertyComparer(PropertyDescriptor prop, ListSortDirection direction)
            {
                ...
                Debug.Assert(CanSort(prop.PropertyType),
                ...
                {
                    var property = typeof(Comparer<>).MakeGenericType(new Type[] { prop.PropertyType }).GetProperty("Default");
                    _comparer = (IComparer)property.GetValue(null, null);
                    _useToString = false;
                }
                else
                {
                    Debug.Assert(CanSortWithToString(prop.PropertyType),
                                 "Cannot use PropertyComparer unless it can be compared by IComparable or ToString");
